/* ============================================================================
   app.js — wires the five screens together.

      intro → video → birthday → "one more thing" → final
                                        ↕
                                  infinite zoom
   ========================================================================= */
...
(function () {
  ...
  function navigateToSection(id) {
    ...
    if (id === 'birthday') {
      window.Messages.startRotator($('#rotator'), C.ROTATING);
    } else {
      window.Messages.stopRotator();
    }
  ...
  function initBirthday() {
    ...
    window.Messages.initNoteRow($('#wishRow'), 5);
  }

  function startBirthday() {
    const scene = scenes.birthday;
    requestAnimationFrame(() => scene.classList.add('is-ready'));
    window.Stickers.scatter(scene, { count: 3, notes: true, slots: [0, 3, 5] });
  }
  ...
  function initFinal() {
    ...
    window.Stickers.inline($('#finalSticker'), 116);
    window.Stickers.scatter(scenes.final, { count: 2, notes: true, slots: [1, 6] });
  ...
  function initBirthdaySite() {
    ...
    window.Messages.initNotePopup($('#noteCard'));
    window.Stickers.preload();
